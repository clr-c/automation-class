...
def test_sample_page():
    duration = 15  # Adjust this to match your test duration
    record_thread = threading.Thread(target=record_screen, args=("test_recording.mp4", duration))
    record_thread.start()

    driver = webdriver.Chrome()
    try:
        file_path = Path("data/sample-exercise.html").resolve()
